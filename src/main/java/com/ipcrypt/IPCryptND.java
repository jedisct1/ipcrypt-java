...
public class IPCryptND {
    ...
    private static byte[] shiftRows(byte[] state) {
        ...
        result[0] = state[0];   result[1] = state[5];   result[2] = state[10];  result[3] = state[15];
        result[4] = state[4];   result[5] = state[9];   result[6] = state[14];  result[7] = state[3];
        result[8] = state[8];   result[9] = state[13];  result[10] = state[2];  result[11] = state[7];
        result[12] = state[12]; result[13] = state[1];  result[14] = state[6];  result[15] = state[11];
        return result;
    }
    ...
    private static byte[] invShiftRows(byte[] state) {
        byte[] result = new byte[16];
        result[0] = state[0];   result[1] = state[13];  result[2] = state[10];  result[3] = state[7];
        result[4] = state[4];   result[5] = state[1];   result[6] = state[14];  result[7] = state[11];
        result[8] = state[8];   result[9] = state[5];   result[10] = state[2];  result[11] = state[15];
        result[12] = state[12]; result[13] = state[9];  result[14] = state[6];  result[15] = state[3];
        return result;
    }
    ...
    private static byte[] mixColumns(byte[] state) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            newState[i*4] = (byte) (MUL2[s0] ^ MUL3[s1] ^ s2 ^ s3);
            newState[i*4+1] = (byte) (s0 ^ MUL2[s1] ^ MUL3[s2] ^ s3);
            newState[i*4+2] = (byte) (s0 ^ s1 ^ MUL2[s2] ^ MUL3[s3]);
            newState[i*4+3] = (byte) (MUL3[s0] ^ s1 ^ s2 ^ MUL2[s3]);
        }
        return newState;
    }
    ...
    private static byte[] invMixColumns(byte[] state) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int result0 = (mul0E((byte)s0) ^ mul0B((byte)s1) ^ mul0D((byte)s2) ^ mul09((byte)s3)) & 0xFF;
            int result1 = (mul09((byte)s0) ^ mul0E((byte)s1) ^ mul0B((byte)s2) ^ mul0D((byte)s3)) & 0xFF;
            int result2 = (mul0D((byte)s0) ^ mul09((byte)s1) ^ mul0E((byte)s2) ^ mul0B((byte)s3)) & 0xFF;
            int result3 = (mul0B((byte)s0) ^ mul0D((byte)s1) ^ mul09((byte)s2) ^ mul0E((byte)s3)) & 0xFF;
            
            newState[i*4] = (byte) result0;
            newState[i*4+1] = (byte) result1;
            newState[i*4+2] = (byte) result2;
            newState[i*4+3] = (byte) result3;
        }
        return newState;
    }
